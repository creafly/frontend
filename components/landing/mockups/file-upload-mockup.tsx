...
type Phase = "idle" | "hovering" | "uploading" | "processing" | "complete";

const demoFiles = [
	{ name: "brand-guidelines.pdf", icon: IconFileTypePdf, size: "2.4 MB", color: "#ef4444" },
	{ name: "logo-pack.zip", icon: IconFile, size: "8.1 MB", color: "#6366f1" },
...
export function FileUploadMockup({ isActive }: FileUploadMockupProps) {
	...
	useEffect(() => {
		...
		const timer = setTimeout(() => setPhase("hovering"), 800);
		return () => clearTimeout(timer);
	}, [isActive, resetDemo]);
	...
	return (
		<div className="h-full flex flex-col">
			...
			<div className="flex-1 p-4 bg-background/50 rounded-b-lg overflow-hidden">
				<motion.div
					className={cn(
						"relative h-32 rounded-xl border-2 border-dashed transition-colors flex flex-col items-center justify-center gap-2",
						phase === "hovering"
					...
					animate={phase === "hovering" ? { scale: [1, 1.02, 1] } : {}}
					transition={{ duration: 0.5, repeat: phase === "hovering" ? Infinity : 0 }}
				>
					<AnimatePresence mode="wait">
						{phase === "idle" && (
							...
							>
								<div className="h-10 w-10 rounded-full bg-muted flex items-center justify-center">
									<Icon icon={IconUpload} size="md" className="text-muted-foreground" />
								</div>
						...
						{phase === "hovering" && (
							...
							>
								...
								>
									<Icon icon={IconUpload} size="lg" className="text-primary" />
								</motion.div>
